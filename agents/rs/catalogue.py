...
def _build_catalogue() -> list[Runbook]:
    ...
    items: list[Runbook] = [
        ...
        _runbook(
            "discover_chaos_experiments",
            name="Discover active fault-injection experiments",
            tool_name="chaos_list_experiments",
            parameter_template={"namespace": "-A"},
            applicable_fault_types=("all",),
            prerequisites=(),
            risk="low",
            mutating=False,
            description=(
                "Enumerate active Chaos Mesh experiments to obtain the exact resource "
                "name every stop_*_chaos runbook requires. Without this evidence step "
                "chaos_resource_name is unknown and all chaos remediation stays blocked."
            ),
            stage="diagnostic",
        ),
    ]
