"""Convert preserved incident trajectories into SFT examples.

The historical live generator was retired: it applied Chaos without a governed
preflight and used cluster-wide cleanup. Use the frozen Train-split corpus builder
in ``training.build_sft_dataset`` for reproducible local data preparation.
"""
...
def main() -> None:
    raise SystemExit(
        "Legacy live trajectory generation is disabled; use training.build_sft_dataset "
